...
function loadGender() {
	$.ajax({
		...
		async: false,
		url: 'http://localhost:2500/common/commondata/getGenders',
...
function loadListOfMedicalComplainType() {
	 console.log('loadListOfMedicalComplainType method is executed');
	$.ajax({
		type: 'POST',
		url: "http://127.0.0.1:2500/common/commondata/getAllCheifComplaint",
		data: JSON.stringify({
...
function loadListOfPoliceComplainType() {
	console.log("loadListOfPoliceComplainType method executed");
	var strUrl ="http://127.0.0.1:2500/common/commondata/getAllCheifComplaint";   
...
function onCheckChange() {
    ...
    if ($("#medicalComplaintChkId").is(':checked')) {
        console.log('Medical check box checked');
        $("#medicalComplainTypeId").prop('disabled', false).trigger("chosen:updated");
    } else {
        console.log('Medical check box unchecked');
...
function onCheckChangeForPolice() {
    ...
    if ($("#policeComplaintChkId").is(':checked')) {
        console.log('Police check box checked');
        $("#policeComplainTypeId").prop('disabled', false).trigger("chosen:updated");
    } else {
        console.log('Police check box unchecked');
...
function onCheckChangeForFire() {
    ...
    if ($("#fireComplaintChkId").is(':checked')) {
        console.log('Fire check box checked');
        $("#fireComplainTypeId").prop('disabled', false).trigger("chosen:updated");
    } else {
        console.log('Fire check box unchecked');
  ...
  console.log(JSON.stringify(addPatientJObject));
   ...
   if(rtnValidationStatus==true||rtnValidationStatus=='true'||rtnValidationStatus =="true"){
	  console.log("inside if-condo")
	   $.ajax({
			type: 'POST',
			url: "http://127.0.0.1:2706/api/dispatch_con/insertDispatch",
...
function resetDispatchForm(){
	console.log('resetDispatchForm method is executed');
...
function decript(){
	 ...
	 var getMailId = window.location.href;
		console.log("@@@@@@@@@@@@@@@@@::"+getMailId);
		var arrayValue =  getMailId.split("?");
		console.log(arrayValue[1]);//hwlJH9nRPKnaM/cc4FB5rw==
		var strUrl ="127.0.0.1:2706/api/version_1/getDescriptedId?id="+arrayValue[1];
		$.ajax({
			type: 'POST',
			url: strUrl,
			dataType: 'json',
