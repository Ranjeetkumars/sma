		...
		console.log("Status" + textStatus);
			...
			// console.log('obj='+obj);
			    JSON.parse(obj).forEach(item => {
			    	// console.log("item.message_bodyitem.message_body="+item.message_body);getListOfMailBasedOnId
...
function openMail(mail_id,from){
	...
	JSON.parse(obj).forEach(item => {
    	 if(item.mail_id==mail_id){
    		 console.log("inside-if condition of openmail javascript function");
    		 console.log("fromName="+item.fromName);
    		 console.log("fromMail="+item.fromMail);
    		 console.log("subject="+item.subject);
    		 console.log("messageBody="+item.messageBody); 
    		 console.log("messageBody="+item.messageBody); 
    		 console.log("createddtm="+item.createddtm); 
    		 ...
    		 if(item.mailStatus=="Unread"){
    			 console.log('mail status is unread')
    			 $.ajax({
    			      type: 'POST',
    			      url: "http://localhost:2706/getMailDetailsController/insertMailStatus",
    			      data: JSON.stringify({
    			    	  "mail_id":mail_id,
    			    	  "mailStatus":1
    			  		}),
...
function getListOfMailBasedOnId(mail_id){
	...
	$.ajax({
	      type: 'POST',
	      url: "http://localhost:2706/getMailDetailsController/getListOfMailDetailsBasedOnMailId",
	      data: JSON.stringify({
	    	  "mail_id":mail_id,
	    	}),
	      success:function(data, textStatus, jqXHR) {
...
function replyMail(){
    ...
    console.log('reply body::'+newReplyBody);
    console.log("newReplyBody::"+newReplyBody.length);
    ...
    {
       console.log('print summer note data='+newReplyBody);
     $.ajax({
     type: 'POST',
     url: "http://localhost:2706/getMailDetailsController/replyMail",
     data: JSON.stringify({
...
function summerNoteValidation(){
	console.log('summerNoteValidation method is executed');
	var code = $('#replyContent1').summernote('code');
	
	console.log("codecodecodecode:::->"+code);// replyToMailBody <p><br></p>
...
function SendTemplate(){
	...
	console.log("SendTemplate method is executed::"+mailId);
	var replyToMailId = document.getElementById('senderMailId').innerHTML;
	var replyToSubject = document.getElementById('senderSubject').innerHTML;
	if(replyToMailId==null || replyToMailId=="" || replyToMailId=='' || replyToMailId=="undefined"
		...
		$.ajax({
		     type: 'POST',
		     url: "http://localhost:2706/getMailDetailsController/sendTemplate",
		     data: JSON.stringify({
		    	 "mail_id":mailId,
